    ...
    <div className="min-h-screen bg-[#0b1326] font-manrope text-[#dae2fd] pb-32">
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-24 bg-[#0b1326]/70 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#4e74ff] to-[#2e5bff] border-2 border-white/10 flex items-center justify-center shadow-lg"><span className="material-icons text-white">analytics</span></div>
          <span className="text-xl font-black text-white uppercase italic">NTT COMMAND CENTER (东努)</span>
        </div>
        <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest">
          <div><p className="opacity-40">Start</p><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-[#131b2e] border border-white/10 text-white rounded-xl px-4 py-2 outline-none focus:border-[#2e5bff]" /></div>
          <div><p className="opacity-40">End</p><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-[#131b2e] border border-white/10 text-white rounded-xl px-4 py-2 outline-none focus:border-[#2e5bff]" /></div>
        </div>
      </header>
               ...
               <table className="w-full text-left min-w-[1000px]">
                  <thead>
                     <tr className="bg-white/5 text-[#8c9bbd] text-[10px] font-black uppercase tracking-widest">
                        ...
                        <th className="px-8 py-6">Staff (Role & Name)</th>
                     ...
                     {filteredSales.map((s) => (
                        <tr key={s.id} className="hover:bg-white/5 transition-colors group text-xs text-white">
                           ...
                           <td className="px-8 py-6">
                              <p className="font-bold text-white mb-1">{s.staff_name || '-'}</p>
                              <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${s.staff_role?.toLowerCase().includes('promotor') ? 'bg-blue-500/20 text-blue-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                                 {s.staff_role || 'Staff'}
                              </span>
                           </td>
        ...
        {activeTab === 'analytics' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-[#131b2e] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
              <h3 className="text-white font-black text-xl uppercase italic mb-8">Revenue Share per Store</h3>
              <div className="w-full h-[400px]">
                {storeChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={storeChartData} innerRadius={100} outerRadius={140} paddingAngle={5} dataKey="omzet" stroke="none">
                        {storeChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={index === 0 ? '#2e5bff' : index === 1 ? '#4e74ff' : '#1e2a4a'} />)}
                      </Pie>
                      <Tooltip contentStyle={{ backgroundColor: '#0b1326', border: 'none', borderRadius: '1rem' }} />
                    </PieChart>
                  </ResponsiveContainer>
                ) : <div className="h-full flex items-center justify-center opacity-20"><p className="font-bold uppercase tracking-widest">No transaction records in this range</p></div>}
              </div>
            </div>
            <div className="bg-[#131b2e] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
              <h3 className="text-white font-black text-xl uppercase italic mb-8">Team Performance Ranking</h3>
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                {picRanking.length > 0 ? picRanking.map((r, i) => (
                  <div key={i} className="flex justify-between items-center p-5 bg-white/5 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black text-[#2e5bff]">#{i+1}</span>
                      <span className="text-sm font-bold text-white uppercase">{r.email.split('@')[0]}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-black text-[#2e5bff]">Rp {format(r.omzet)}</p>
                      <p className="text-[10px] font-bold text-[#8c9bbd]">{r.units} Units</p>
                    </div>
                  </div>
                )) : <div className="py-20 text-center opacity-20 uppercase font-black tracking-widest text-xs">No personnel data available</div>}
              </div>
            </div>
          </div>
        )}
      ...
      <nav className="fixed bottom-0 w-full h-24 bg-[#131b2e]/80 backdrop-blur-2xl border-t border-white/5 flex justify-around items-center px-10 pb-6 z-50">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-col items-center justify-center transition-all ${activeTab === tab.id ? 'text-[#2e5bff] scale-110' : 'text-[#8c9bbd] hover:text-white'}`}>
      ...
      {loading && <div className="fixed inset-0 bg-[#0b1326]/60 backdrop-blur-md flex items-center justify-center z-[100]"><div className="w-12 h-12 border-4 border-[#2e5bff] border-t-transparent rounded-full animate-spin"></div></div>}
    </div>
  )
}
