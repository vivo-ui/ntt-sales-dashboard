...
export default function InputPage() {
  ...
  const fetchData = async () => {
    ...
    // CRITICAL FILTER: Filter stores by column 'pic' matching first part of email (e.g. 'JO')
    ...
    <div className="min-h-screen bg-[#0b1326] font-manrope text-[#dae2fd] pb-32">
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-20 bg-[#0b1326]/70 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="text-right">
          <p className="text-[8px] font-bold text-[#dae2fd]/40 uppercase tracking-widest">Logged in as</p>
          <p className="text-[10px] font-bold text-[#2e5bff]">{userEmail}</p>
        </div>
      </header>
      ...
      <main className="pt-28 px-6 space-y-8 max-w-lg mx-auto">
        <div className="bg-[#131b2e]/60 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-[#0b1326] border border-white/5 rounded-2xl px-5 py-4 text-sm font-bold text-white outline-none focus:border-[#2e5bff]/50"
                />
                <button className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 bg-[#2e5bff]/10 px-3 py-1.5 rounded-lg border border-[#2e5bff]/20">
                  <span className="material-icons text-xs text-[#2e5bff]">photo_camera</span>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between bg-[#0b1326] border border-white/5 rounded-2xl px-6 py-4">
                 <button onClick={() => setFormData({...formData, qty: Math.max(1, formData.qty - 1)})} className="text-[#2e5bff]"><span className="material-icons">remove_circle_outline</span></button>
                 <span className="text-xl font-black text-white">{formData.qty}</span>
                 <button onClick={() => setFormData({...formData, qty: formData.qty + 1})} className="text-[#2e5bff]"><span className="material-icons">add_circle_outline</span></button>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            disabled={loading}
            className="w-full py-5 bg-gradient-to-br from-[#4e74ff] to-[#2e5bff] text-white rounded-[1.5rem] font-black text-lg shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
          >
