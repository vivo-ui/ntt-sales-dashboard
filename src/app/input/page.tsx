  ...
  const startScanner = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // ENHANCED CAMERA LOGIC: Prioritize Primary Back Camera
        try {
          const cameras = await Html5Qrcode.getCameras();
          if (cameras && cameras.length > 0) {
            // Find the most appropriate camera (Back/Environment)
            const primaryBackCamera = cameras.find(c => 
            ...
            // Fallback to generic environment mode
            await html5QrCode.start(
          ...
          console.error("Primary scan attempt failed", scanErr);
        ...
        alert("Gagal mengakses kamera utama.");
        setIsScanning(false);
      }
    }, 300);
  }
    ...
    <div className="min-h-screen bg-[#0b1326] font-manrope text-[#dae2fd] pb-32">
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-20 bg-[#0b1326]/70 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xl font-black tracking-tight text-white uppercase italic">Data Acquisition</span>
        </div>
      ...
      <main className="pt-28 px-6 space-y-8 max-w-lg mx-auto">
          ...
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col items-center justify-center p-6">
            <div className="w-full max-w-sm space-y-6 text-center">
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic">Primary Camera (1)</h3>
                <p className="text-sm text-[#dae2fd]/60 italic">Position the barcode within the central frame</p>
              </div>
        ...
        <div className="bg-[#131b2e]/60 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8 relative overflow-hidden">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">Target Store</label>
              <select 
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">Staff Role</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">Staff Full Name</label>
                <input 
                  type="text"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">Product Model</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">IMEI 1 Identifier</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#8c9bbd] ml-1">Quantity</label>
